...
const playerNum=0;//human is white
let computer;
let playerList;
let currPlayer; //number
let board;
let messageSlot;
let again,undo,redo,highlightButton; //buttons
let boardimg;
let ChessImages;
let moveLine;
let hist;
...
function initializeVars() {
	...
	moveLine=false;
	computer=playerList[1-playerNum];
}
...
function makeDOM() {
	...
	undo.position(425+windowWidth/2-width/2,60+windowHeight/3-height/2);
	undo.mousePressed(undoMove);
	redo=createButton("Redo");
	redo.position(425+windowWidth/2-width/2,90+windowHeight/3-height/2);
...
function mousePressed() {
	if(!promote&&currPlayer==playerNum) {
		...
		if(loc&&checkMate==-1) {
			if(loc.piece&&loc.piece.player.num==currPlayer) {
				...
				if(currPiece&&currPiece.canMoveTo(loc)) {
					...
					currPiece=false;
					redraw();
					if(checkMate==-1&&computer.getAllMoves().length!=0&&!promote) {//not checkmate and not stalemate
						computerMove();
						playerList[1-currPlayer].updateAllMoves();
						playerMessage();
					}
...
function promotePawn(val) {
	if(currPlayer==playerNum) {//computer already moved
		undoMove();
	}
...
function shouldRotate() {
	return playerNum;//if player is black it is 1/true
}

function computerMove() {
	let moves=computer.getAllMoves();
	let move=random(moves);
	completeMove(move.movedPiece,move.endLoc,"computer");	
}